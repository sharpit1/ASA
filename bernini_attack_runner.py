...
def main() -> int:
    ...
    try:
        ...
        raise RuntimeError(
            "failed to import ImageNet_Compatible from ImageNet_Compatible_Dataset.py"
        ) from exc

    batchsize = max(1, int(cfg.batchsize))
    image_dataset = ImageNet_Compatible(root=str(dataset_root), image_size=int(cfg.image_size))
    data_loader = DataLoader(image_dataset, batch_size=batchsize, shuffle=False, drop_last=False)

    image_ids, _, _ = load_nips_ground_truth(dataset_root)
    if len(image_dataset) != len(image_ids):
        print(
            "[bernini_runner] WARNING: dataset length mismatch: "
            f"ImageNet_Compatible={len(image_dataset)} csv_ids={len(image_ids)}; using minimum length."
        )
    total_available = min(1000, len(image_ids), len(image_dataset))
    ...
    try:
        ...
        for batch_idx, (_images, label_ori_batch, label_tar_batch) in enumerate(data_loader):
            base_idx = batch_idx * batchsize
            batch_len = int(label_ori_batch.shape[0])
            for in_batch_idx in range(batch_len):
                ...
                true_label = int(label_ori_batch[in_batch_idx].item())
                target_label = int(label_tar_batch[in_batch_idx].item())
