...
# Attack every clean-correct sample with VLM mode and no strategy slots.
...
RUN_NAME="${RUN_NAME:-firered_vlm_res_clean_correct_full_vlm_none_$(date +%Y%m%d_%H%M%S)}"
...
echo "[firered_full_vlm_none] project_root=$PROJECT_ROOT"
echo "[firered_full_vlm_none] config=$CONFIG_PATH"
echo "[firered_full_vlm_none] output_dir=$OUTPUT_ROOT/nips2017/resnet50/$RUN_NAME"
echo "[firered_full_vlm_none] candidate_range=0-$((CANDIDATE_COUNT - 1))"
echo "[firered_full_vlm_none] selection=all_clean_correct"
echo "[firered_full_vlm_none] attack_mode=vlm strategies=none"
echo "[firered_full_vlm_none] run_name=$RUN_NAME"
