...
class BerniniRenderSession:
    ...
    def render(
        ...
        *,
        ...
        normalized = [
            image if image.size == base_size else image.resize(base_size, Image.LANCZOS)
            for image in images
        ]
        out_path = Path(output_path)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        strip = Image.fromarray(np.hstack([np.asarray(image.convert("RGB")) for image in normalized]))
        strip.save(out_path)
...
class BerniniAttackRuntimeAdapter:
    ...
    def evaluate_and_candidate(self, **kwargs):
        ...
        with tempfile.TemporaryDirectory(prefix="bernini_strategy_and_") as tmpdir:
            tmp_root = Path(tmpdir)
            wave_idx = 0
            while remaining_candidates and query_count < query_budget:
                ...
                batch_path = tmp_root / f"trial_batch_{int(wave_idx):03d}.png"
                try:
                    render_session.render(prompts=trial_prompts, output_path=str(batch_path))
                    module = self._get_module()
                    tiles = module.split_prompt_strip(batch_path, num_prompts=len(trial_prompts))
                    if len(tiles) != len(trial_prompts):
                        raise RuntimeError(f"tile_count_mismatch:{len(tiles)}!={len(trial_prompts)}")
                except Exception as exc:
                    failures.append(f"batch_{int(wave_idx)}:{type(exc).__name__}:{exc}")
                    break

                best_improvement: Optional[Dict[str, object]] = None
                for plan, trial_image in zip(trial_plans, tiles):
                    try:
