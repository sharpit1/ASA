...
_THIRD_PARTY_HF_DIFFUSERS = Path(__file__).resolve().parent / "third_party" / "hf_diffusers_git"
...
class Flux2KleinRenderSession:
    ...
    def render(
        ...
        *,
        ...
        strip = Image.fromarray(np.hstack([np.asarray(image.convert("RGB")) for image in images]))
